...
const formatPeriod = (periodInSeconds: number): string => {
  ...
  for (const formattedUnit of [formattedDays, formattedHours, formattedMins, formattedSecs]){
    formattedPeriod = formattedPeriod && formattedUnit
      ? `${formattedPeriod}, ${formattedUnit}` : `${formattedPeriod}${formattedUnit}`
  }
...
export const formatTrigger = (trigger: Trigger): string => {
  ...
  const formattedTrigger: string = `${trigger.Statistic} ${trigger.MetricName} ${trigger.ComparisonOperator} \
${trigger.Threshold} for ${trigger.EvaluationPeriods} period(s) of ${period}.`
...
export const formatMoreInfoUrl = (region: string, alarmName: string): string => {
  ...
  const formattedUrl =
    `https://console.aws.amazon.com/cloudwatch/home?region=${region}#alarm:alarmFilter=ANY;name=${encodedAlarmName}`
