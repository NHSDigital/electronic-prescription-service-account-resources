...
// This is a version of TypescriptLambdaFunction similar to the one in eps-cdk-constructs
// But as this is in account-resources, we do not need to import values from account-resources
// so instead, the values are passed into the construct
...
export interface TypescriptLambdaFunctionProps {
  ...
  /**
   * splunk delivery stream for lambda log subscription filter
   */
  readonly splunkDeliveryStream?: CfnDeliveryStream
  /**
   * splunk subscription filter role for lambda log subscription filter
   */
  readonly splunkSubscriptionFilterRole?: Role
  /**
   * Managed policy to allow using lambda insights log group
   */
  readonly lambdaInsightsLogGroupPolicy: ManagedPolicy
  /**
   * Managed policy to allow encrypting with the cloudwatch logs kms key
   */
  readonly cloudwatchEncryptionKMSPolicy: ManagedPolicy
  /**
   * Flag to indicate whether to add splunk subscription filter
   */
...
export class TypescriptLambdaFunction extends Construct {
  ...
  public constructor(scope: Construct, id: string, props: TypescriptLambdaFunctionProps) {
    ...
    const LAMBDA_INSIGHTS_LAYER_ARNS = {
    ...
    const insightsLayer = LayerVersion.fromLayerVersionArn(
      scope, "insightsLayer", insightsLambdaLayerArn)
    ...
    if (addSplunkSubscriptionFilter && splunkDeliveryStream && splunkSubscriptionFilterRole) {
